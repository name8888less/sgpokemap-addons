...
var mankeyIvsGL = mankeyGL.map(function(item){
    return item.iv
});

var mankeyLvlsGL = mankeyGL.map(function(item){
    return item.lvl
});
...
var mankeyIvsUL = mankeyUL.map(function(item){
    return item.iv
});

var mankeyLvlsUL = mankeyUL.map(function(item){
    return item.lvl
});
    ...
    for (var i = 0; i < pokemons.length; ++i) {
        ...
        if ((ivPercentage >= min_iv_compare && hasValidStats) || shouldTurnFilterOff()) {
            ...
            if (currentPokemon.id==56) {
                ...
                var index = mankeyIvsGL.indexOf(statsString);
                var pvpString = "";
                if (index != -1) {
                    //found. check level
                    targetPokemonLevel = mankeyLvlsGL[index];
