...
$("#close_donation_button").click(), $(".toast-close-button").click(), $("#overlay").hide(), $("#topbar").hide(), $(".please").hide(), $("#map").css({
    width: "100vw",
    height: "100vh",
    top: 0,
    bottom: 0
...
isMobile && ($(".leaflet-control-zoom").css("display", "none"), $("#custom_panel").css("top", "10px"));
...
if (isFirstTime) {
	...
	$("#filter_label").after('<div id="pvp_filter"><input type="checkbox" id="pvp_filter_check" name="pvp_filter_check"><label for="pvp_filter_check">PvP</label> <input id="pvp_top_number" type="search" placeholder="PvP Top Ranks" value="30"/><div id="pvp_filter_list"></div></div>');
	$("body").append('<style>#pvp_filter {padding: 10px;} #pvp_filter_list {display:none; border: 1px solid #ccc; margin: 10px 100px 10px 0; padding: 10px;} .pvp_mon_checkbox {margin-bottom: 10px;} pvp_mon_checkbox:last {margin-bottom: 0px;} .pvp_iv_list_rank {display: block; clear: both;} .pvp_iv_hit_container {margin:0; padding:0;} #pvp_top_number {width: 100px}</style>');
	perfTiming = false;
	isFirstTime = false;
}
			        ...
			        for (var j = 0; j < ivsCheckList.length; j++) {
			            ...
			            if (currentPokemon.id==pvpMonId) {
							for (var k = 0; k < pvpIvListArray.length; k++) {
								...
								if (currentPokemon.attack == pvpIvItem.atk && currentPokemon.defence == pvpIvItem.def && currentPokemon.stamina == pvpIvItem.sta && currentPokemon.level <= pvpIvItem.lvl)
								{
