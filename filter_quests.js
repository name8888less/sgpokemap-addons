...
if (isFirstTime) {
    ...
    var clearButton = $('<button>Clear All</button>');

    // Position the button absolutely within the filter box
    clearButton.css({
        position: 'absolute',
        top: '50px',
        right: '10px', // Adjust right padding as needed
        padding: '5px 10px', // Adjust button padding as desired
    });

    // Append the button to the filter box
...
function refreshQuests() {
  ...
  // override the original processNewFilter and add new ui elements to it
  // Store the original function
var originalProcessNewFilter = processNewFilter;

// Redefine the global function
processNewFilter = function(...args) {
    // get the values of the added elements
    var filterText = $("#mon_filter").val();    

    // Call the original function
    var result = originalProcessNewFilter.apply(this, args);

    // render the new UI elements
    addUiToFilterBox();

    //reinstate the filter text
    $("#mon_filter").val(filterText).trigger("change");   
